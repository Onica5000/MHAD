...
List<pw.Page> buildCombinedPages({
  ...
  // -------------------------------------------------------------------------
  // One MultiPage flows all content across as many pages as needed. This
  // avoids the fixed-page `Spacer` gaps (no large unused white space) and lets
  // the body font match the official form's readable ~11pt size.
  // -------------------------------------------------------------------------
  ...
  return [
    pw.MultiPage(
      ...
      build: (ctx) => [
        ...
        // ---- Part II — Treatment Preferences (Facility + Medications) ----
        ...
        pw.Text('4. Preferences for experimental studies', style: boldStyle()),
        pw.SizedBox(height: 4),
        if (prefs != null) ...[
          checkRow(
        ...
        // 5. Drug trials
        pw.Text('5. Preferences for drug trials', style: boldStyle()),
        pw.SizedBox(height: 4),
        if (prefs != null) ...[
          checkRow(
        ...
        // ---- Part III C (Authority) + Part IV (Guardian) ----
        ...
        pw.SizedBox(height: 4),
        // Resolve display fields based on the Phase-2 `guardianRelation`
        // choice so 'sameAsPrimary' / 'sameAsAlternate' render the named
        // agent rather than emitting a blank nominee block.
        ...() {
