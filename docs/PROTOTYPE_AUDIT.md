...
| m-clinician | *(none — dropped)* | ❌ | **Dropped 2026-06-17.** Never built. A clinician-summary screen is redundant against — and risks misrepresenting — the full signed directive / PDF, which is the authoritative clinician-facing artifact. (The earlier ✅ here was inaccurate; no `clinician/` code ever shipped.) |
